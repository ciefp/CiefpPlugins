#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpPlugins/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpPlugins

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpPlugins
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpPlugins
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpPlugins/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpPlugins-main/usr' '/'
set +e
cd
sleep 2
